...
import base.tool.ip.IPLocation;
...
public class IpUtil {
    ...
    private Hashtable ipCache;// 用来做为cache，查询一个ip时首先查看cache，以减少不必要的重复查找
    private RandomAccessFile ipFile;// 随机文件访问类
    private MappedByteBuffer mbb;// 内存映射文件
    // 单一模式实例
    ...
    private final String IPDATE_FILE_PATH = getClassPath(this.getClass());
    ...
    public static void main(String[] args) {
        //System.out.println(new IpUtil().getCountry("59.108.104.83"));
        //System.out.println(new IpUtil().getArea("59.108.104.83"));

//        String str = new IpUtil().getClass().getClassLoader().getResource(".").getPath()+"QQWry3.Dat";
//        File file = new File(str);
//        //System.out.println(file.exists());
//        //System.out.println( new IpUtil().getClass().getClassLoader().getResource(".").getPath());

        System.exit(0);
    }

    public String getCountryArea(String ip){
        String location = getCountry(ip) + " " + getArea(ip) + "[" + ip + "]";
        return location;
    }
    ...
    public IpUtil() {
        ...
        b3 = new byte[3];
        try {
            ipFile = new RandomAccessFile(IPDATE_FILE_PATH + IPDATE_FILE, "r");  //实际用取消注释
     ...
     *
     * @param s 地点子串
     * @return 包含IpBean类型的List
     */
    public List getIPEntriesDebug(String s) {
        List ret = new ArrayList();
        long endOffset = ipEnd + 4;
        for (long offset = ipBegin + 4; offset <= endOffset;
             offset += IP_RECORD_LENGTH) {
            // 读取结束IP偏移
    ...
    public List getIPEntries(String s) {
        ...
        try {
            ...
            for (int offset = (int) ipBegin + 4; offset <= endOffset;
                 offset += IP_RECORD_LENGTH) {
    ...
    private String readString(long offset) {
        try {
            ipFile.seek(offset);
            int i;
            for (i = 0, buf[i] = ipFile.readByte(); buf[i] != 0;
